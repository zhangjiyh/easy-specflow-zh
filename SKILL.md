...
3. `plan`：拆出可执行步骤、修改范围、验证模式、验证方式和完成标准，更新 `执行计划.md`。
4. `execute`：按计划连续推进实现，默认从 Step 1 执行到 Step N，直到全部已承诺步骤完成或遇到阻塞，并把过程、影响文件、验证命令或手动验证反馈与结果写入 `进度记录.md`。
...
3. `执行计划.md` 中每个关键步骤都必须写清验证模式、验证方式和完成标准；验证模式默认需要让用户在自动验证、手动验证、混合验证中确认。
...
7. 如果用户选择手动验证，agent 不主动执行未经确认的自动验证，只提供模块级检查建议；用户反馈必须写入 `验证记录.md`，反馈引发的修改必须回到 `execute` 记录改动并追加复验轮次。
8. `accept` 必须基于 `任务说明.md`、`代码库地图.md`、`执行计划.md`、`进度记录.md`、`验证记录.md` 已存在的事实与证据给出结论。
9. 只有当以下条件同时满足时，才允许将 `state.json.status` 标记为 `done`：
   - `执行计划.md` 没有未勾选步骤。
   - `任务说明.md` 中的验收标准已按结果回填。
   - `验证记录.md` 中没有待补验证项或未关闭失败项。
   - `验收记录.md` 中的关键检查全部通过。
   - 最终结论为“通过”，而不是“有条件通过”或“不通过”。
10. 如果结论是“有条件通过”或仍存在阻塞，只能把任务收敛到 `accept`，不能写 `done`。
11. `state.json` 是当前任务阶段和状态的唯一事实来源；文档里的阶段字段只表示文档所属阶段，不表示当前任务实时状态。

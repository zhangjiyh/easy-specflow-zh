...
3. 在 `plan` 阶段补全 `执行计划.md`，确保每步都有验证模式、验证方式和完成标准。
4. 在 `execute` 阶段把实际改动和验证结果写入 `进度记录.md`。
5. 在 `verify` 阶段先确认自动验证、手动验证或混合验证；用户选择手动验证时，只给模块级检查建议并等待反馈。
6. 汇总 `验证记录.md`，不要只写“已验证”；手动反馈触发修改时，先回到 `execute` 记录修改，再追加复验。

...
3. 在 `plan` 阶段补全 `执行计划.md`，并明确验证模式为自动验证、手动验证或混合验证。
4. 在 `execute` 阶段把代码改动摘要、影响文件和验证结果写入 `进度记录.md`。
5. 在 `verify` 阶段补全 `验证记录.md`；如果用户选择手动验证，只提出模块级检查建议并记录用户反馈。
6. 在 `accept` 阶段继续使用同一套文档，不再生成新的平台私有文件。

如果手动验证反馈触发后续修改，应回到 `execute` 记录修改，再回到 `verify` 追加复验轮次，不能直接进入 `accept`。

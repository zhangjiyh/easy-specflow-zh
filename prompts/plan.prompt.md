# plan 阶段提示词

## 目标

把当前任务拆成可执行、可验证、可回溯的步骤。

## 输入

1. `任务说明.md`
2. `代码库地图.md`
3. `state.json`
4. `rules/workflow.md`
5. `rules/stage-transition.md`
6. `templates/执行计划.md`

## 输出

1. 完整的 `执行计划.md`
2. 明确的步骤勾选列表
3. 风险与依赖说明
4. 更新后的 `state.json`

## 要求

1. 使用中文。
2. 步骤要直接对应实现动作。
3. 每一步都要有修改范围、验证模式、验证方式和完成标准。
4. `执行计划.md` 中的勾选项将作为后续流程判断依据，必须可真实回填。
5. 只输出计划，不要在同一轮把步骤假定为已完成。
6. 准备进入执行阶段时，将 `state.json.stage` 保持为 `plan`，或在真正开始第一个步骤时更新为 `execute`。
7. 验证模式必须在自动验证、手动验证、混合验证中明确；如果验证会涉及外部系统、真实数据、部署、账号权限、较高成本或用户明确偏好手动验证，必须请用户确认后再执行。
8. 如果选择手动验证，只提供模块级检查建议，不生成过细操作清单；用户反馈前不得把手动验证视为通过。

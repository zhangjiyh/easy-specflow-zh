...
3. `plan`
   基于 `spec` 与 `repo-map` 拆分步骤，并为每一步写清修改范围、执行动作、验证模式、验证方式、完成标准和当前状态，对应 `执行计划.md`。
4. `execute`
   按计划连续执行剩余步骤，默认从 Step 1 推进到 Step N，直到全部已承诺步骤完成或遇到阻塞；把每一步的改动、影响文件、验证命令或手动反馈、验证结果和阻塞写入 `进度记录.md`。
5. `verify`
...
3. 没有带验证模式和验证方式的 `plan`，`execute` 就缺少约束。
...
不同平台适合执行的验证动作不同，用户也可能更希望在 `verify` 阶段采用手动验证。因此协议要求：进入正式验证前，先确认本轮验证模式。

1. `自动验证`：agent 执行测试、构建、lint、脚本、浏览器检查等自动验证，并记录命令和结果。
2. `手动验证`：agent 不主动执行未经确认的自动验证，只提出模块级检查建议，由用户验证后反馈结果。
3. `混合验证`：低风险自动检查由 agent 执行，业务流程、外部系统、真实账号或体验判断由用户手动确认。

手动验证建议应保持模块级，例如登录与权限主链路、表单提交与异常提示、列表筛选分页、文件上传下载、移动端关键布局等，不应生成过细的逐点击清单。

用户反馈是验证证据的一部分。如果手动验证反馈触发了细节修改，任务必须回到 `execute` 记录修改，再回到 `verify` 追加复验轮次；没有复验闭环时，不得进入 `accept` 或标记 `done`。
...
3. `执行计划.md` 中的每一步都必须先写清验证模式、验证方式和完成标准，才算计划完成。
...
7. 手动验证反馈触发修改后，必须回到 `execute` 并追加复验轮次，不能直接收尾。
8. `accept` 不是主观收尾，而是对前序证据做最终结论。
9. 只要仍有阻塞、待补验证或“有条件通过”，就不能把 `status` 标记为 `done`。

...
4. `execute`
   按计划逐步修改并记录实际结果，把每一步的改动、影响文件、验证命令、验证结果和阻塞写入 `进度记录.md`。
5. `verify`
...
1. 一次只推进一个明确阶段，或 `execute` 中的一个明确步骤。
2. `repo-map` 是正式阶段，不是 README 里的可选建议。
3. `执行计划.md` 中的每一步都必须先写清验证方式，才算计划完成。
4. `execute` 阶段完成步骤后，必须同步更新计划状态和执行记录。
5. `verify` 不能凭空新增事实，只能汇总前面步骤留下的执行与验证证据。
6. `accept` 不是主观收尾，而是对前序证据做最终结论。
7. 只要仍有阻塞、待补验证或“有条件通过”，就不能把 `status` 标记为 `done`。
